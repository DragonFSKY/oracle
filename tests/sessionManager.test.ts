...
describe("session lifecycle", () => {
  ...
  test("serializes concurrent metadata patches without losing either update", async () => {
    const meta = await sessionModule.initializeSession(
      { prompt: "Concurrent update", model: "gpt-5.2-pro", mode: "browser" },
      "/tmp/cwd",
    );

    await Promise.all([
      sessionModule.updateSessionMetadata(meta.id, { status: "running" }),
