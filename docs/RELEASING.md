...
- [ ] Rebuild macOS notifier helper with signing + notarization:
  ...
  - Keep the Sparkle ed25519 private key outside the repository; export `SPARKLE_PRIVATE_KEY_FILE` to its machine-local path whenever the build script needs to sign an appcast/enclosure.
