...
- Research mode: set `browserResearchMode:"deep"` for broad public-web research and cited reports. Use `gpt-5.6` + `browserThinkingTime:"pro"` for the current GPT-5.6 Sol model with the unified Pro effort tier, `gpt-5.5-pro` + `browserThinkingTime:"extended"` for the legacy Pro alias, or `gpt-5.5` + `browserThinkingTime:"heavy"` when you explicitly want Thinking Heavy.
