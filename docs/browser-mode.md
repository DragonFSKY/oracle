...
- `--model`: the same flag used for API runs is accepted, but the ChatGPT automation path also supports browser-only GPT-5.6 aliases. Use `gpt-5.6` for the current GPT-5.6 default or `gpt-5.6-sol` to pin the `GPT-5.6 Sol` picker entry. Existing GPT-5.5, GPT-5.4, and GPT-5.2 variants remain available. Legacy Pro aliases still resolve to the latest Pro picker target.
