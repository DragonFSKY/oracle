...
- **ChatGPT attach-running mode** (GPT-\* models): Oracle attaches to your already-running local Chrome session through Chrome’s local remote-debugging toggle, opens a dedicated tab (or a top-level window with `browser.newWindow`), and leaves the browser process/profile alone.
...
2. **Automation stack** – code lives under `src/browser/`:
   ...
   - Attach-running mode reads local `DevToolsActivePort` metadata for the selected local port, connects to the browser websocket, opens a dedicated tab or configured top-level window, and reuses the same DOM automation/capture flow against that attached browser.
...
- `--browser-max-concurrent-tabs`: soft limit for simultaneous ChatGPT targets sharing one profile (default `3`; AdsPower defaults to `1` unless explicitly configured). Set `ORACLE_BROWSER_MAX_CONCURRENT_TABS` for a per-host default; explicit CLI/config values win. Additional runs wait up to the browser timeout for a slot and log `[browser] Waiting for ChatGPT browser slot...`.
...
- `--browser-thinking-time <light|standard|extended|heavy|pro>`: set the ChatGPT thinking-time intensity. Use `pro` to select the unified Intelligence picker's top Pro tier independently of the GPT model version. You can also set a default in `~/.oracle/config.json` via `browser.thinkingTime`.
...
When Codex, Claude Code, or another Oracle caller share the same browser profile, each run acquires a target slot before opening ChatGPT. The general default allows three simultaneous targets; shared AdsPower profiles conservatively default to one unless `browser.maxConcurrentTabs` is set explicitly. Later callers wait instead of failing because another agent is already using the browser. This is most useful for long Pro/Thinking runs where one agent may wait for a response while another agent needs to start a separate consult.
