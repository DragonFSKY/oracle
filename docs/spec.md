...
- Files have no default per-file size cap. Set `ORACLE_MAX_FILE_SIZE_BYTES` or `maxFileSizeBytes` to a positive byte count only when an explicit guard is desired; `0` means unlimited.
