...
Every operator client offers **System**, **简体中文**, and **English**. System chooses Simplified Chinese for any Chinese OS/browser locale and English otherwise; English is the fallback. Web saves the choice in `localStorage`, macOS in `UserDefaults`, Windows in `%LocalAppData%/OracleRelay/settings.json`, and Android in the existing `oracle-relay` `SharedPreferences`. Language changes apply immediately on web/Android and on the next reliable UI rebuild on desktop.

Operator wording is maintained only in [`locales/operator/`](../locales/operator/). Contributors edit the JSON catalog, run `pnpm run locales:operator:generate`, then run `pnpm run locales:operator:check`; generated platform adapters are reviewed but never hand-edited.
