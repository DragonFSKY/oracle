...
- Relay/operators: add persisted System, Simplified Chinese, and English UI choices across Web and native operator clients. All client chrome, status labels, validation, and notifications use one generated catalog while Relay protocol values and user content remain unchanged.
- Relay/operators: add native Windows and Android clients with environment-provided endpoint/credentials, task claiming and lease renewal, SHA-256 verified attachment handling, clipboard/share actions, abort flow, Markdown/file response upload, notifications, and platform-native background behavior. Add GitLab CI jobs that verify Node and produce credential-free `oracle-relay-*` Windows and Android artifacts, plus an optional macOS runner job.
- Relay/MCP: add the dedicated `dragon-relay-mcp` stdio server with only `ask_expert` and `await_expert`. Expert calls block on authenticated SSE without producer-side status polling, survive Relay restarts through persisted snapshots and bounded reconnects, use caller-known idempotency keys to prevent duplicate human tasks, serialize local finalization, preserve colliding response attachment names, and acknowledge server cleanup only after an atomic local result commit.
- Relay: add a durable human-operated `relay` engine for cross-device manual ChatGPT/Claude/Gemini consultations, including a persistent task server, responsive operator web client, native always-on-top macOS operator with automatic attachment caching and compact waiting mode, request/response attachment transfer with SHA-256 verification, task claiming/expiry, operator-side cancellation for stuck work, CLI integration, one-shot result recovery, `oracle relay-serve`, and a hardened Node 24 Docker deployment with loopback-only publishing, health checks, persistent storage, automatic restart, non-logging environment credentials, and secret-safe JSON lifecycle diagnostics. Native relay CLI submissions upload and publish the task, return a durable session ID, and stop without a local worker; a later `dragon-relay wait <id>` checks once and restores completed output. The operator UI can copy text/code attachments and supported images directly to the clipboard without saving them first, while retaining downloads for arbitrary binary files; it collects only the answer and optional returned files, omitting unverifiable provider/model self-reporting. Existing browser CLI callers can switch transparently with `browser.transport: "relay"` or `ORACLE_BROWSER_TRANSPORT=relay` while avoiding all Chrome automation, including inherited/restarted sessions.
- Relay: stream staged request attachments as retryable 4 MiB chunks instead of embedding them in Base64 JSON, with configurable 512 MiB per-file and 1 GiB per-task defaults, checksum and size verification before publication, and streamed operator downloads. Delete server-side task files after the producer has safely persisted the answer, with a 24-hour retention fallback for unacknowledged terminal tasks.
- Browser/MCP: add an allow-listed composer-tool activation layer with `--browser-tool web-search` and MCP `browserTools`, including active-pill verification immediately before prompt submission.
...
- MCP: continue saved ChatGPT conversations with `consult.followupSession`, expose generated dry-run bundles as structured output, inherit AdsPower profile pins for follow-ups, and let `await_session` recover saved browser conversations without CLI fallback.
- MCP: run local consultations in detached durable workers so Codex/Claude or MCP server restarts do not cancel the underlying session; let `await_session` safely claim and reattach submitted browser sessions whose controller exited; fence worker hand-off and serialize atomic session metadata writes across processes; fail closed when a worker exits before submission; and use a read-only stale-run watchdog to capture completed answers without ever clicking ChatGPT's "Answer now" control or resubmitting the prompt.
- Relay/CLI: add the dedicated `dragon-relay` CLI for submitting, inspecting, and resuming human Relay tasks, plus `dragon-relay-continue` for carrying task context into a clean interactive Codex thread.
- Skills/distribution: version `ask-pro`, `ask-pro-repomix`, and `ask-pro-zip` with the Relay fork, package them as a GitLab artifact, and add English/Chinese installation, architecture, self-hosting, CI/CD, security, and contribution documentation for the `main` branch.

### Fixed

- Relay/MCP: make `ask_expert` and `await_expert` deadline-free across the MCP schema, SSE reconnect loop, remote task lifetime, recovered-session metadata, and local loopback response receiver. Codex hosts use a practically non-expiring tool allowance, so one MCP call stays blocked and the same agent turn resumes immediately when the operator returns an answer.
- Relay/MCP: remove the implicit 1 MiB CLI/MCP file guard plus the Relay and loopback 512 MiB-per-file/1 GiB-per-task guards. Attachment guards now default to unlimited and accept `0` as the explicit unlimited value; positive CLI/config/server values remain available as opt-in safety guards, while chunk sizing and small JSON control-plane caps remain independent.
- MCP/macOS: restore executable modes on every generated CLI entry point after TypeScript builds, preventing direct private-checkout deployments from making `dragon-relay-mcp` fail at startup with `Permission denied (os error 13)`.
- Relay/macOS: let operators paste Finder files and clipboard images directly into the response editor as return attachments with **⌘V** or Windows-style **Ctrl+V**, handling the shortcut in the editor instead of relying on an absent standard Edit menu. Add a visible clipboard-attachment fallback, preserve plain-text paste behavior, and clean generated temporary image files with the task cache.
- Relay/operators: stage response attachments by filename, size, and SHA-256, upload them in retryable chunks, and atomically publish the answer only after every file verifies. Reuse the same upload ID for an identical retry so clicking submit again resumes verified progress instead of deleting partial data, use 1 MiB public chunks with a five-minute per-chunk allowance on slow Mac links, and retain 512 KiB chunks for loopback delivery. The macOS client no longer sends images as one Base64 JSON request, shows stable per-file upload state that polling cannot overwrite, and retains drafts and attachments after failures for one-click retry.
- Relay/MCP/macOS: advertise a task-scoped loopback receiver with a random port and one-time 256-bit capability when the blocking MCP and native operator are on the same Mac. Refresh that capability whenever `ask_expert` or `await_expert` resumes an existing task, so an MCP restart cannot leave the operator pointing at a stale local port. Verify the task identity, transfer return attachments directly into the originating session with the same chunk and SHA-256 guarantees, keep the public Relay as the control plane, and automatically fall back to public chunk uploads whenever the probe or local transfer is unavailable. The native Mac client now uses a dedicated no-proxy URL session for Relay traffic instead of inheriting slow or unstable system HTTP/SOCKS proxies.
- Relay/macOS: turn compact mode into a two-row action bar with direct multi-task switching, prompt copy, conditional one-or-many request-attachment copy, safe text-only **Paste and submit**, refresh, expand, and hide controls. Refuse task switches while a response draft, return attachment, or upload is active so content cannot leak across tasks.
- Relay/macOS: restore task-scoped pasted response images from the temporary cache after an app restart or when returning to that task, preventing an installed update from silently dropping a prepared image from the response list.
- Relay/operators: remove per-device task ownership locks so authenticated Mac, Windows, Android, and web clients can operate the same active task together, including marking submitted, renewing, responding, and aborting from a different device. Keep the existing claim fields only as backward-compatible activity metadata, and discard stale local task details when another device has already completed or removed the task.
- Relay/native clients: add explicit macOS shrink-to-menu-bar buttons to both expanded and compact floating windows, wire the native yellow window control to the same action, and preserve free manual resizing. Rebuild the Windows layout with DPI-aware elastic headers, ellipsized metadata, scroll-safe minimum content sections, screen-constrained restored frames, and smaller usable window bounds; automatically clear 404/409 stale tasks and prevent overlapping attachment downloads from fighting over one temporary file.
- Relay/macOS: stop the five-second task refresh from repeatedly overwriting a user-moved floating-window position. No-op mode refreshes now leave the frame untouched, while real compact/expanded transitions preserve the current window's top-left position when screen space allows instead of jumping to the other window's saved anchor. Respect explicit shrink-to-menu-bar and red-close actions across new-task and status refreshes; only an explicit **显示 Oracle Relay** action may reveal the window again.
- Relay/Windows: bypass stale per-user localhost proxies for the configured public Relay HTTPS endpoint, and install login auto-start through an interactive scheduled task so SSH deployments leave the operator window running after the remote shell exits.
- Repository/build: make the GitHub Fork's `main` branch the maintained Relay source, keep the package unpublished, add secret-safe multi-platform build entry points, build/push immutable Relay container images through the optional GitLab pipeline, and provide a serialized manual production deploy. Remove hard-coded native operator credentials from the tracked source; macOS/Android receive local build settings and Windows reads the current user's environment.
- Relay/macOS: make both compact and expanded native operator windows movable and resizable, restore the standard macOS zoom control, remember window frames across launches, add menu actions for zoom/default sizing, and relax rigid content-height constraints so smaller windows remain usable.
- Relay/MCP: recover interrupted submissions by caller request ID, resume already committed multi-chunk upload prefixes, hold session finalization locks for the full blocking wait, retry pending acknowledgements on completed-session fast paths, and reject idempotency-key reuse when attached file contents change. Remove the legacy browser Oracle MCP implementation and generated surface entirely; Codex/Claude bridge configuration now installs only the two-tool Dragon Relay MCP. Daye consultations now share the same nine-section prompt validator and bounded independent-review workflow as Ask Pro.
- Skills/Relay: prefer the long-blocking Dragon Relay MCP for Ask Pro, Repomix, ZIP, review cycles, and Daye research. While a tool call is pending agents stop and wait for the SSE result. If MCP is not exposed or fails before task creation, perform a matching dry-run followed by one asynchronous `dragon-relay` CLI submission; later continuations check the original stable ID once without polling or duplicate tasks.
- Relay/CLI: do not report an asynchronous submission until every attachment is uploaded and the server confirms the task is `queued`; surface upload failures immediately instead of leaving an invisible `uploading` task. Add a production Nginx template with request buffering and long body timeouts for large cross-device uploads.
- Relay/CLI: stop local execution after asynchronous publication instead of spawning a detached polling worker. `dragon-relay wait <id>` now performs exactly one server check, returns immediately while pending, and restores the answer only after completion.
- Skills/Relay: remove mandatory sensitive-file, secret-content, and nested-archive blockers from Ask Pro Repomix/ZIP packaging while retaining manifest generation, SHA verification, and Relay dry-runs.
- Skills/Relay: make Ask Pro, Repomix, ZIP, and Daye submissions truly asynchronous by default. Return the durable slug and end the turn immediately without starting `wait`, `status`, background terminals, or polling; each later user-requested continuation runs one `dragon-relay wait` check and stops again if the task is still pending.
- Skills/AskPro: require every live consultation to use a fully populated nine-section Chinese prompt covering the task, facts, completion criteria, non-completion cases, scope lock, investigation and falsification, evidence rules, stop conditions, and output contract, enforced by a deterministic pre-submit validator. Add repository-specific review modules and a bounded workflow that freezes post-change evidence, uses a new Relay task for independent regression review, and performs at most one final acceptance pass when needed.
- Codex/Relay: migrate oversized historical threads into a clean interactive session without copying stale tool-unavailable claims, injected Skill/environment snapshots, or obsolete transport diagnostics. Add `dragon-relay-continue --check` to verify shell access and a Relay dry-run without creating a live task.
- Browser/MCP: distinguish transient `/c/WEB:...` submission routes from durable ChatGPT conversation IDs, retain structured route-transition evidence and failure DOM/screenshots, keep monitoring through canonical URL promotion, recover stale sessions by their owned CDP target before stored URLs, require stable turn-scoped completion proof before watchdog finalization, and cleanly finalize the fenced worker without resubmitting. Browser response retries now share one absolute deadline instead of multiplying the configured timeout.
