...
- Skills/distribution: version `ask-pro`, `ask-pro-repomix`, and `ask-pro-zip` with the Relay fork, package them as a GitLab artifact, and add English/Chinese installation, architecture, self-hosting, CI/CD, security, and contribution documentation for the `main-relay` branch.
...
- Repository/build: make the GitHub Fork's `main-relay` branch the maintained Relay source, keep the package unpublished, add secret-safe multi-platform build entry points, build/push immutable Relay container images through the optional GitLab pipeline, and provide a serialized manual production deploy. Remove hard-coded native operator credentials from the tracked source; macOS/Android receive local build settings and Windows reads the current user's environment.
