...
import { OPERATOR_LOCALES } from "./operatorLocale.generated.js";
...
const OPERATOR_CATALOG_JSON = JSON.stringify(OPERATOR_LOCALES).replaceAll("<", "\\u003c");
...
<label class="muted"><span data-i18n="language.label"></span><select id="language" onchange="setLanguage(this.value)"><option value="system" data-i18n="language.system"></option><option value="zh-CN" data-i18n="language.zh-CN"></option><option value="en" data-i18n="language.en"></option></select></label>
<section id="login" class="card"><h2 data-i18n="login.title"></h2><input id="token" type="password" data-i18n-placeholder="operator.token"><input id="operator" data-i18n-placeholder="operator.name"><p><button onclick="connect()" data-i18n="connect"></button></p></section>
<section id="queue" class="hidden"><div class="row"><h2 data-i18n="tasks.title"></h2><button class="secondary" onclick="loadTasks()" data-i18n="refresh"></button></div><div id="tasks"></div></section>
<section id="detail" class="hidden"></section></main><script>
const L=${OPERATOR_CATALOG_JSON};let languageSetting=localStorage.relayLanguage||'system';
function locale(){return languageSetting==='zh-CN'||languageSetting==='en'?languageSetting:/^zh(?:[-_]|$)/i.test(navigator.language||'en')?'zh-CN':'en'}
function t(key,values={}){return (L[locale()][key]||L.en[key]||key).replace(/\{([A-Za-z][A-Za-z0-9_]*)\}/g,(_,name)=>String(values[name]??'{'+name+'}'))}
function statusLabel(status){return Object.hasOwn(L.en,'status.'+status)?t('status.'+status):status}
function applyLanguage(){document.documentElement.lang=locale();language.value=languageSetting;document.querySelectorAll('[data-i18n]').forEach(e=>e.textContent=t(e.dataset.i18n));document.querySelectorAll('[data-i18n-placeholder]').forEach(e=>e.placeholder=t(e.dataset.i18nPlaceholder));if(active)updateDetailLanguage();else if(!queue.classList.contains('hidden'))loadTasks().catch(()=>{})}
function setLanguage(value){languageSetting=['system','zh-CN','en'].includes(value)?value:'system';localStorage.relayLanguage=languageSetting;applyLanguage()}
...
token.value=auth;document.getElementById('operator').value=operator;applyLanguage();if(auth)connect();
...
async function connect(){auth=token.value.trim();operator=document.getElementById('operator').value.trim()||'operator';localStorage.relayToken=auth;localStorage.relayOperator=operator;try{await loadTasks();login.classList.add('hidden');queue.classList.remove('hidden');if(Notification.permission==='default')Notification.requestPermission();const requested=new URLSearchParams(location.search).get('task');if(requested)await openTask(requested)}catch(e){alert(t('connection.failed',{message:e.message}))}}
async function loadTasks(){const list=await (await api('/v1/tasks')).json();tasks.innerHTML=list.length?list.map(task=>'<div class="card task" onclick="openTask(\''+task.id+'\')"><span class="badge">'+esc(statusLabel(task.status))+'</span><h3>'+esc(task.title)+'</h3><div class="muted">'+esc(t('task.model',{model:task.modelHint||'—'})+' · '+t('task.files',{count:task.attachments.length}))+' · '+new Date(task.createdAt).toLocaleString(locale())+'</div></div>').join(''):'<div class="card muted">'+esc(t('tasks.empty'))+'</div>';if(list.some(task=>task.status==='queued')&&Notification.permission==='granted')new Notification(t('new.task'))}
...
function attachmentActions(a){const kind=attachmentKind(a),copy=kind==='text'?'<button data-i18n="copy.content" onclick="copyAttachment(\''+a.id+'\')">'+esc(t('copy.content'))+'</button> ':kind==='image'?'<button data-i18n="copy.image" onclick="copyAttachment(\''+a.id+'\')">'+esc(t('copy.image'))+'</button> ':'';return copy+'<button data-i18n="download" class="secondary" onclick="downloadFile(\''+a.id+'\')">'+esc(t('download'))+'</button> '}
function render(){queue.classList.add('hidden');detail.classList.remove('hidden');detail.innerHTML='<button data-i18n="back" class="secondary" onclick="back()">'+esc(t('back'))+'</button><div class="card"><span class="badge" data-detail="status">'+esc(statusLabel(active.status))+'</span><h2>'+esc(active.title)+'</h2><div data-detail="model" class="muted">'+esc(t('task.model',{model:active.modelHint||'—'}))+'</div><h3 data-i18n="prompt">'+esc(t('prompt'))+'</h3><button data-i18n="copy.prompt" onclick="copyPrompt()">'+esc(t('copy.prompt'))+'</button> <button data-i18n="mark.submitted" class="secondary" onclick="markSubmitted()">'+esc(t('mark.submitted'))+'</button> <button data-i18n="abort" class="danger" onclick="abortTask()">'+esc(t('abort'))+'</button><pre>'+esc(active.prompt)+'</pre><h3 data-i18n="attachments">'+esc(t('attachments'))+'</h3><div class="files">'+(active.attachments.length?active.attachments.map(a=>'<p>'+attachmentActions(a)+esc(a.displayPath)+' ('+Math.ceil(a.sizeBytes/1024)+' KB)</p>').join(''):'<p data-i18n="attachments.none" class="muted">'+esc(t('attachments.none'))+'</p>')+'</div></div><div class="card"><h3 data-i18n="answer">'+esc(t('answer'))+'</h3><textarea id="answer" data-i18n-placeholder="answer.placeholder" placeholder="'+esc(t('answer.placeholder'))+'"></textarea><p data-i18n="response.files.input">'+esc(t('response.files.input'))+'</p><input id="responseFiles" type="file" multiple><button data-i18n="answer.submit" onclick="submitResponse()">'+esc(t('answer.submit'))+'</button></div>';updateDetailLanguage()}
function updateDetailLanguage(){if(!active||detail.classList.contains('hidden'))return;const status=detail.querySelector('[data-detail="status"]'),model=detail.querySelector('[data-detail="model"]');if(status)status.textContent=statusLabel(active.status);if(model)model.textContent=t('task.model',{model:active.modelHint||'—'});detail.querySelectorAll('[data-i18n]').forEach(e=>e.textContent=t(e.dataset.i18n));detail.querySelectorAll('[data-i18n-placeholder]').forEach(e=>e.placeholder=t(e.dataset.i18nPlaceholder))}
async function copyPrompt(){await navigator.clipboard.writeText(active.prompt);alert(t('copy.success'))}
...
async function abortTask(){if(!confirm(t('abort.message')))return;try{active=await (await api('/v1/tasks/'+active.id+'/abort',{method:'POST',body:JSON.stringify({operator})})).json();back()}catch(e){alert(t('connection.failed',{message:e.message}))}}
function findAttachment(id){return active.attachments.find(a=>a.id===id)}
async function copyAttachment(id){const item=findAttachment(id);if(!item)return;try{const r=await api('/v1/tasks/'+active.id+'/attachments/'+id),blob=await r.blob(),kind=attachmentKind(item);if(kind==='text'){await navigator.clipboard.writeText(await blob.text());alert(t('attachment.copied-file',{filename:item.filename}));return}if(kind==='image'){if(!window.ClipboardItem)throw new Error(t('error.image-unsupported'));const bitmap=await createImageBitmap(blob),canvas=document.createElement('canvas');canvas.width=bitmap.width;canvas.height=bitmap.height;canvas.getContext('2d').drawImage(bitmap,0,0);bitmap.close?.();const png=await new Promise((ok,no)=>canvas.toBlob(value=>value?ok(value):no(new Error(t('error.image-conversion'))),'image/png'));await navigator.clipboard.write([new ClipboardItem({'image/png':png})]);alert(t('attachment.copy-image-file',{filename:item.filename}));return}throw new Error(t('error.file-copy-unsupported'))}catch(e){alert(t('error.copy-failed',{message:e.message}))}}
