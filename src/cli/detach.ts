...
export function shouldDetachSession({
  ...
  // Relay tasks are durable on the server. The default submit-only path must not
  // leave a local worker behind; explicit blocking runs stay in the foreground.
  if (engine === "relay") return false;
